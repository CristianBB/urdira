...
const actionTelemetry = `\n\n## Codex action telemetry\n\nCompleted web-search and file-change actions are reported separately and do not contribute to MCP, shell, repository-read, or context-character metrics.\n\n| Web searches | File changes | First action | Integration warnings | Hook errors | Unclassified actions |\n|---:|---:|---:|---:|---:|---:|\n| ${measurement.web_search_calls} | ${measurement.file_change_actions} | ${measurement.first_action_type ?? "—"} | ${measurement.integration_warning_count} | ${measurement.hook_error_count} | ${measurement.unclassified_action_count} |\n`;
...
writeFileSync(`${output}.md`, markdown.replace("\n\n## Adoption and limitations", `${actionTelemetry}\n## Adoption and limitations`));
