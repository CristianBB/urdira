...
async function lexicalDocumentRow(opened: WorkspaceDatabase, artifactVersionId: string): Promise<{ readonly valid_from_generation: number; readonly valid_to_generation: number | null; readonly content_hash: string } | undefined> {
  return opened.database.get<{ valid_from_generation: number; valid_to_generation: number | null; content_hash: string }>("SELECT valid_from_generation, valid_to_generation, content_hash FROM lexical_documents WHERE artifact_version_id = ?", [artifactVersionId]);
}

describe("reconcileLexicalProjection", () => {
  it("creates documents and FTS5 entries for every visible text-decodable version, skips binary versions, marks the generation complete, and is idempotent on immediate re-run; then closes stale documents and inserts replacements after a generation bump", async () => {
    ...
    await withWorkspace(workspaceId, async (opened, cas) => {
      ...
      expect(await opened.database.get<{ artifact_id: string }>("SELECT artifact_id FROM lexical_fts WHERE artifact_version_id = ? AND lexical_fts MATCH ?", ["artv-1", '"alpha"'])).toEqual({ artifact_id: "art-1" });
      ...
      // The closed document keeps only typed metadata; source text remains in
      // CAS and is re-read through its content hash when needed.
      const closedRow = await lexicalDocumentRow(opened, "artv-1");
      expect(closedRow?.valid_to_generation).toBe(2);
      expect(closedRow?.content_hash).toMatch(/^sha256:/u);

      const replacementRow = await lexicalDocumentRow(opened, "artv-1-v2");
      expect(replacementRow).toMatchObject({ valid_from_generation: 2, valid_to_generation: null });
  ...
  it("preserves a UTF-8 BOM when rebuilding the lexical projection", async () => {
    const workspaceId = "ws-lexical-bom";
    await withWorkspace(workspaceId, async (opened, cas) => {
      const text = "\uFEFF<?xml version=\"1.0\"?><project />";
      await seedTextVersion(opened, cas, workspaceId, { artifactId: "art-bom", artifactVersionId: "artv-bom", text, validFromGeneration: 1 });
      await setCurrentGeneration(opened, workspaceId, 1);
      ...
      expect(result).toEqual({ generation: 1, closed: 0, inserted: 1, skipped_oversized: 0, skipped_undecodable: 0, marker_written: true });
      expect(await lexicalDocumentRow(opened, "artv-bom")).toMatchObject({ content_hash: expect.stringMatching(/^sha256:/u) });
      await expect(opened.projections.searchLiteral("project")).resolves.toHaveLength(1);
    });
  });
