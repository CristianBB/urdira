...
function buildInstructions(): string {
  ...
  return [
    ...
    "1. Call urdira_index_status with exactly {workspace_root:\"/absolute/repository/root\"} and optional response_budget. Do not send api_version, scope, options, or query fields; those belong to query/context tools. The separate status-list and cursor forms use only the documented workspace_ids, cursor, include_* flags, and response_budget fields.",
...
export function buildBenchmarkInstructions(discoveryPath?: string): string {
  ...
  const lines = [
    ...
    "Resolve the explicit workspace_root with urdira_index_status before querying; never infer scope from process state. The bootstrap call is exactly {workspace_root:<repository root>} with optional response_budget. Do not send api_version, scope, options, or query fields to urdira_index_status; those belong to query/context tools. Its separate status-list and cursor forms use only the documented workspace_ids, cursor, include_* flags, and response_budget fields.",
