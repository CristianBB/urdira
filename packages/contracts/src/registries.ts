...
const operationRegistryEntries = [
  ...
  entry("core:semantic_affected_page", "Plan 2026-09-06 (Frente S-A): pages through the documents a semantic materialization pass has not yet covered, keyed by the coverage view's affected_artifact_set_id.")
];
...
const operationErrorRegistryEntries = [
  ...
  entry("core:ipc_timeout", "Registered core operation error core:ipc_timeout."),
  ...
  entry("core:affected_set_stale", "Plan 2026-09-06 (Frente S-A, R11): a core:semantic_affected_page cursor or affected_artifact_set_id names a set that no longer matches the workspace's current affected documents; never served as a mixed/partial page."),
  entry("core:selector_unresolvable", "Frente Q-2 (2026-09-08): a record-scoped selector carries an identity_id/identity_key form the native v4 structural store has no dedicated index for; rejected outright rather than served by an O(corpus) linear scan."),
  entry("core:non_subject_operation", "Frente Q-2 (2026-09-08): a status/administrative operation (e.g. core:index_status) was named as a query-pipeline or recipe stage; it never emits ResultSubject-shaped stream items, so it cannot be pushed down or bound like a subject-producing operation and is rejected before it reaches the generic full-corpus fallback.")
];
...
export interface OperationDefinition {
  ...
  /** Minimum indexed frontier needed for a complete operation result. */
  required_frontier: "source" | "syntax" | "structural" | "semantic";
  /** Structural stage ordinal when the operation is not source-only. */
  required_stage: 0 | 1 | 2 | 3;
  description: string;
...
const operationErrorDetails: Readonly<Record<string, readonly string[]>> = {
  ...
  "core:selector_unresolvable": ["workspace_id?", "unresolved_ids"],
  "core:non_subject_operation": ["operation_id", "reason_code"]
};
...
const errorPolicy: Readonly<Record<string, { retryable: boolean | "conditional"; recovery: readonly string[] }>> = {
  ...
  "core:ipc_timeout": { retryable: true, recovery: ["retry_operation", "inspect_daemon_status"] },
  ...
  "core:affected_set_stale": { retryable: false, recovery: ["reexecute_query", "inspect_semantic_coverage"] },
  "core:selector_unresolvable": { retryable: false, recovery: ["correct_selector", "discover_definitions", "inspect_completeness"] },
  "core:non_subject_operation": { retryable: false, recovery: ["correct_pipeline", "inspect_index_status"] },
};
...
const payloadProperty = (code: string, name: string): PayloadPropertySchema => {
  ...
  const booleanNames = new Set(["safe_automatic_restart", "fallback_attempted", "transaction_rolled_back", "candidate_set_complete", "retryable"]);
...
const operationArgumentLogicalTypes: Readonly<Record<string, string>> = {
  ...
  affected_artifact_set_id: "Text", cursor: "Text", limit: "PositiveInteger",
};
...
const publicQueryArgumentDescriptions: Readonly<Record<string, string>> = {
  ...
  affected_artifact_set_id: "The affected-document set to page through, from a semantic coverage view's affected_artifact_set_id.", cursor: "Opaque continuation token from a previous page's next_cursor or previous_cursor.", limit: "Maximum affected documents to return in this page.",
};
...
const operationSpecs: Readonly<Record<string, { schema: string; fields: readonly ModelFieldContract[]; streams: readonly string[]; scopes: readonly ("single_workspace" | "comparison")[]; batchable: readonly string[] }>> = {
  ...
  "core:get_outline": { schema: "core:GetOutlineArguments", fields: [operationArgumentField("container", "required", "OutlineContainerSelector"), operationArgumentField("depth", "optional"), operationArgumentField("include_non_public", "optional"), operationArgumentField("filter", "optional")], streams: ["members", "pending_sites"], scopes: ["single_workspace", "comparison"], batchable: [] },
  ...
  "core:search_text": { schema: "core:SearchTextArguments", fields: [operationArgumentField("pattern"), operationArgumentField("syntax", "optional"), operationArgumentField("case_sensitive", "optional"), operationArgumentField("word_mode", "optional"), operationArgumentField("filter", "optional"), operationArgumentField("result_projection", "optional"), operationArgumentField("subjects", "optional")], streams: ["matches", "subjects"], scopes: ["single_workspace", "comparison"], batchable: ["subjects"] },
  ...
  "core:semantic_affected_page": { schema: "core:SemanticAffectedPageArguments", fields: [operationArgumentField("affected_artifact_set_id"), operationArgumentField("cursor", "optional"), operationArgumentField("limit", "optional")], streams: ["semantic_affected_artifacts"], scopes: ["single_workspace"], batchable: [] },
};

/**
 * Query admission metadata is part of the operation registry.  The daemon
 * must not maintain a second hard-coded operation-to-readiness table.
 */
const operationFrontiers: Readonly<Record<string, { readonly required_frontier: OperationDefinition["required_frontier"]; readonly required_stage: OperationDefinition["required_stage"] }>> = {
  "core:find_artifacts": { required_frontier: "source", required_stage: 0 },
  "core:search_text": { required_frontier: "source", required_stage: 0 },
  "core:get_source": { required_frontier: "source", required_stage: 0 },
  "core:discover_definitions": { required_frontier: "syntax", required_stage: 1 },
  "core:find_records": { required_frontier: "syntax", required_stage: 1 },
  "core:get_outline": { required_frontier: "syntax", required_stage: 1 },
  "core:resolve_symbol": { required_frontier: "structural", required_stage: 2 },
  "core:find_references": { required_frontier: "structural", required_stage: 2 },
  "core:expand_relations": { required_frontier: "structural", required_stage: 2 },
  "core:find_paths": { required_frontier: "structural", required_stage: 2 },
  "core:analyze_impact": { required_frontier: "structural", required_stage: 3 },
  "core:find_related_tests": { required_frontier: "structural", required_stage: 3 },
...
const operationResultSubjectTypes: Readonly<Record<string, readonly string[]>> = {
  ...
  "core:semantic_affected_page": ["artifact"],
};
const operationStreamFields: Readonly<Record<string, Readonly<Record<string, readonly string[]>>>> = {
  ...
  "core:get_outline": { members: ["subject", "containment_depth", "source_span"], pending_sites: ["path", "start", "end", "site_kind", "reason", "source_id", "stable_sort_key"] },
  ...
  "core:semantic_affected_page": { semantic_affected_artifacts: ["affected_page"] },
};
const operationStreamClassifications: Readonly<Record<string, Readonly<Record<string, readonly string[]>>>> = {
  ...
  "core:get_outline": { members: ["confirmed"], pending_sites: ["unclassified"] },
  ...
  "core:semantic_affected_page": { semantic_affected_artifacts: ["unclassified"] },
};
const operationStreamItemSchemas: Readonly<Record<string, Readonly<Record<string, string>>>> = {
  ...
  "core:get_outline": { members: "core:ResultSubject", pending_sites: "core:PendingSiteView" },
  ...
  "core:semantic_affected_page": { semantic_affected_artifacts: "core:SemanticAffectedArtifactPage" },
};
...
export const operationDefinitions: readonly OperationDefinition[] = operationRegistryEntries.map((operation) => {
  ...
  return {
    ...
    ...(operationFrontiers[operation.id] ?? { required_frontier: "structural", required_stage: 3 }),
...
export const operationErrorDefinitions: readonly OperationErrorDefinition[] = operationErrorRegistryEntries.map((error) => ({
  ...
  retryable_default: errorPolicy[error.id]?.retryable ?? (error.id === "core:ipc_timeout" ? true : (() => { throw new Error(`Missing operation-error policy for ${error.id}`); })()),
  recovery_actions: errorPolicy[error.id]?.recovery ?? (error.id === "core:ipc_timeout" ? ["retry_operation", "inspect_daemon_status"] : (() => { throw new Error(`Missing operation-error recovery for ${error.id}`); })()),
