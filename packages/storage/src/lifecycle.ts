...
const MIGRATION_TABLE_NAMES = [
  ...
  // Plan 2026-09-06 (Frente S-A): `semantic_document_status` (`packages/storage/sql/workspace-v3.sql`)
  // is a v3-single-file table exactly like `semantic_index_state`/
  // `vector_projection_rows` on this same line -- omitting it here left a
  // v3 workspace's shadow migration/verify path throwing
  // `storage:migration_adapter_missing` for this new table the instant it
  // existed on disk (see `MIGRATION_TABLE_ADAPTERS`'s own doc comment:
  // "every persisted table has a table-specific ... adapter").
  // Q5 campaign (2026-09-06/08): same gap, same fix, for the two semantic
  // tables the S-F and S-D fronts added after S-A -- `semantic_coverage_summary`
  // (per-generation coverage summary, S-F) and `semantic_segment_cache`
  // (content-addressed segment-vector cache, S-D). `vector_projection_by_owner_idx`
  // (S-G) is an INDEX, not a table, so it needs no adapter entry here; the
  // new `segment_index`/`segment_start`/`segment_end` columns S-B added to
  // `vector_projection_rows` (already registered below) need none either --
  // this migration path is column-agnostic for tables with no payload
  // columns (`MIGRATION_PAYLOAD_COLUMNS[table] ?? []` empty array).
  "lexical_documents", "lexical_index_state", "semantic_index_state", "semantic_document_status", "semantic_coverage_summary", "semantic_segment_cache", "artifact_dependencies", "metric_projections", "vector_shards", "vector_projection_rows",
