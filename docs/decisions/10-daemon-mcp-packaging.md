...
The product exposes MCP directly through `urdira mcp`. That command is the coding agent's single server entry point: it serves MCP on stdio, starts the matching per-user daemon when none exists, or attaches to the compatible daemon already maintaining the selected data root. `urdira web` is a second local composition owned by the CLI, not a daemon protocol: it serves a bundled SPA and the same MCP factory over authenticated loopback Streamable HTTP as defined by [the local web decision](24-local-web-interface.md).

The MCP, CLI, and web boundaries do not open workspace databases, read repositories, load plugins, generate embeddings, issue their own cursors, or infer workspace scope. Internally they delegate durable work to the daemon so watchers, indexes, workers, cursor caches, and concurrent clients survive adapter processes. That delegation channel is a private implementation boundary, not a supported public API.
...
The data root contains a fixed endpoint descriptor and an operating-system lock file. POSIX hosts bind an owner-restricted local socket under that root; Windows derives a deterministic named-pipe identity from the same endpoint path so no filesystem socket is attempted. Startup acquires an exclusive non-advisory process lock where available, binds the local endpoint, and writes a digest-protected descriptor containing the transport protocol version, exact engine build, private-interface version, available RPC capabilities, endpoint identity, process ID, owner identity, and start time. `core:status` repeats the build, interface version, and capabilities.
...
endpoint. Only the foreground `urdira web` composition binds HTTP, and only to
authenticated `127.0.0.1`; it delegates to the daemon's unchanged private IPC.
