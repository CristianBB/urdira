...
Writes stream to a private temporary file in the CAS filesystem, compute digest and length, fsync the file, atomically install it without replacing conflicting content, fsync the containing directory, then publish references in SQLite. An existing key is reused only after length and digest verification. A key containing different bytes is an integrity failure, never an overwrite.
