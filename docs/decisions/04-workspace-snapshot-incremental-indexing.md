...
Only a successful stable `enumerate` or `reconcile` result whose batch has complete scope coverage and deletion authority can infer absence from a missing item. An individually authoritative watch deletion may prove only its named artifact when the provider advertised and registered that event class. Physical watcher bindings may register concrete delete events as authoritative; a rename is processed as an authoritative absence followed by a separate presence generation, preserving the lifecycle barrier. Empty results, partial scans, watcher overflow, source changes during capture, provider unavailability, budget exhaustion, cancellation, and generic failure never prove absence. They preserve the last published source state and drive retry or degraded freshness according to the closed outcome.
...
A physical watcher backend error invalidates that underlying subscription and emits one provider-reset barrier. Urdira closes the failed subscription and serially installs one replacement after bounded exponential backoff. Repeated callbacks from the invalidated subscription are stale and cannot consume additional retry attempts or start concurrent replacements. A successful event resets the consecutive-failure count; bounded exhaustion leaves the periodic authoritative reconciliation as the recovery backstop. On macOS, Urdira selects the native `kqueue` backend explicitly for workspace watchers instead of FSEvents: indexing can keep the Node callback busy long enough for the FSEvents client queue to drop events, while kqueue has no such client queue; a symlinked root's FSEvents-only path canonicalization is separately normalized so it cannot silently lose events either way. This default holds only up to `KQUEUE_FILE_WATCH_BUDGET` (2,000) eligible files under the workspace root: `@parcel/watcher`'s kqueue backend holds one open file descriptor per watched file for the life of the subscription, and above the budget that per-file cost is the confirmed cause of `spawn EBADF` once the daemon tries to fork any other child process, so Urdira trades kqueue's lower latency away for FSEvents above the budget rather than risk process-wide fd exhaustion. Below the budget, kqueue's own detection latency is sub-10ms at repository scale, not the multi-second cost once attributed to it. The normal inclusion exclusions remain applied to both backends, and an explicit `URDIRA_WATCHER_BACKEND=fs-events` override remains available below the budget for investigation.
...
A `reconcile` scan scope (distinct from the source-provider `reconcile` call above, which it uses as its authoritative enumeration) answers a branch switch or `git pull` by diffing that stable enumeration against the current published state through content-hash equivalence: a path whose content digest is unchanged (including a file that only moved, or whose only difference is copies/import order) never enters the candidate pipeline. Below a measured touched-fraction threshold (`RECONCILE_DELTA_THRESHOLD = 0.01`) the incremental delta path processes only the changed set; at or above it, the scan republishes through the full cold pipeline instead, because the delta path's own dependency/graph-root diffing narrows scope in ways not proven equivalent to a from-scratch oracle above that fraction. Both branches are diffed against the identical authoritative enumeration, so `reconcile`'s choice of pipeline never changes the resulting Merkle roots, only which code path computes them. A `git pull` that changes nothing settles in about one second under this path (`docs/evidence/2026-09-06-v4-reconcile-threshold.md` §2: noop reconcile 1.00-1.02s vs a from-scratch cold scan around 20.6-25.0s on the same n8n corpus).
