...
Rational values use a canonical reduced signed-numerator and positive-denominator structure with bounded magnitudes defined by the query contract. This is a schema-defined pair of existing logical integer values, not a new primitive. Addition, multiplication, comparison, ordering, persistence, and explanation use exact arithmetic; binary floating-point values and platform-dependent rounding cannot affect membership or order. A zero contribution is valid only when produced by the declared calibration or missing-value rule, never as an implicit fallback for an extraction failure.
